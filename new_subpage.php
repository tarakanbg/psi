...
<?php
	...
	if (isset($_POST['submit'])) {
		...
		if (empty($errors)) {
			$query = "INSERT INTO ".DB_PREFIX."subpages (
						menu_name, position, visible, content, page_id, description, keywords, editor, author
					) VALUES (
						'{$menu_name}', {$position}, {$visible}, '{$content}', {$page_id}, '{$description}', '{$keywords}', '{$editor}', '{$author}'
					)";
			if ($result = mysql_query($query, $connection)) {
				...
				redirect_to("content.php?subpage={$new_page_id}");
			} else {
...
<table id="structure">
	<tr>
		...
		<td id="page">
			<h2><?php echo $l_adding_subpage; ?></h2>
			<?php if (!empty($message)) {echo "<p class=\"message\">" . $message . "</p>";} ?>
			<?php if (!empty($errors)) { display_errors($errors); } ?>
			
			<form action="new_subpage.php?page=<?php echo $_GET['page']; ?>" method="post">
				<?php $new_page = true; ?>
...
<p><?php echo $l_position; ?><select name="position">
	<?php
		if (!$new_page) {
			$page_set = get_subpages_for_page($sel_subpage['page_id']);
			$page_count = mysql_num_rows($page_set);
		} else {
			$page_set = get_subpages_for_page($sel_page['id']);
...
<p><?php echo $l_keywords . ": "; ?><br />
	<textarea name="keywords" rows="5" cols="60" class="mceNoEditor"></textarea>
				...
				<input type="submit" name="submit" value="<?php echo $l_create_subbpage; ?>" />
			</form>
			<br />
			<a href="content.php"><?php echo $l_cancel; ?></a><br />
		</td>
	</tr>
</table>
